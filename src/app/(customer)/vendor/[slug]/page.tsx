  ...
  return (
    <div className="min-h-[100dvh]">
      <Suspense fallback={<VendorSkeleton />}>
        <AsyncVendorContent slug={slug} category={category} />
      </Suspense>
    </div>
  );
}

async function AsyncVendorContent({ slug, category }: { slug: string; category?: string }) {
  const { vendor, products, productsGroupedByCategory, categories, error } = await getVendorStoreData(slug, category);
