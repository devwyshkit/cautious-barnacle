...
export async function calculateOrderTotalRPC(
  ...
  try {
    ...
    const { data, error } = await supabase.rpc('calculate_order_total', {
      ...
      p_delivery_fee_override: deliveryFeeOverride,
    ...
    return {
      data: {
        ...
        personalization_charges: Number(result.personalization_charges || result.personalizationCharges) || 0,
        delivery_fee: Number(result.delivery_fee || result.deliveryFee) || 0,
        platform_fee: Number(result.platform_fee || result.platformFee) || 0,
        gst: Number(result.gst) || 0,
        discount: Number(result.discount) || 0,
        wallet_discount: Number(result.wallet_used || result.wallet_discount || result.walletUsed || result.walletDiscount) || 0,
