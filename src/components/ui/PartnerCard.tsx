...
export function PartnerCard({ data, className }: PartnerCardProps) {
    const prepTime = data.prep_hours ? formatPrepTime(data.prep_hours) : null;
                ...
                <div className="relative aspect-[4/3] w-full rounded-2xl overflow-hidden bg-zinc-100">
                    {data.image_url ? (
                        ...
                        <div className="flex items-center justify-center h-full bg-zinc-50">
                            <span className="text-xs font-black text-zinc-300 uppercase tracking-[0.2em]">Partner</span>
                        </div>
                    )}
